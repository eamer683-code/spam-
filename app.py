...
.history-row small { color:#444 !important; }
.history-row strong { color:#1a1a2e !important; }
...
def keyword_check(text):
    # normalize EN: lowercase + collapse multiple spaces + remove punctuation for matching
    t = text.lower()
    t_clean = re.sub(r'[^\w\s]', ' ', t)          # استبدال علامات الترقيم بمسافة
    t_clean = re.sub(r'\s+', ' ', t_clean).strip() # توحيد المسافات

    # normalize AR: remove tashkeel + normalize letters
    t_n = re.sub(r'[أإآا]','ا', re.sub(r'ة','ه', re.sub(r'ى','ي',
          re.sub(r'[\u064B-\u065F\u0670]','', t))))
    t_n_clean = re.sub(r'[^\u0600-\u06FF\s]', ' ', t_n)
    t_n_clean = re.sub(r'\s+', ' ', t_n_clean).strip()

    for kw in EN_SPAM_KW:
        if kw in t_clean: return True, kw
    for kw in AR_SPAM_KW:
        kw_n = re.sub(r'[أإآا]','ا', re.sub(r'ة','ه', re.sub(r'ى','ي', kw)))
        if kw_n in t_n_clean: return True, kw
    return False, None
...
@st.cache_resource
def train_model():
    # ── تعديل 5: error handling أوضح لتحميل spam.csv ──────────
    try:
        df_en = pd.read_csv('spam.csv', encoding='latin-1')
        df_en = df_en[['v1','v2']]
        df_en.columns = ['label','message']
    except FileNotFoundError:
        st.warning('⚠️ ملف spam.csv مش موجود — هيتدرب على البيانات العربية فقط.\n'
                   'spam.csv not found — training on Arabic data only.')
        df_en = pd.DataFrame(columns=['label','message'])
    except Exception as e:
        st.warning(f'⚠️ مشكلة في تحميل spam.csv: {e} — هيكمل بدونه.')
        df_en = pd.DataFrame(columns=['label','message'])
    ...
    # ── إزالة الصفوف اللي بقى فيها clean فاضي بعد الـ preprocessing ──
    empty_clean = (df['clean'].str.strip() == '').sum()
    if empty_clean > 0:
        df = df[df['clean'].str.strip() != '']

    # ── تعديل 5: معالجة NaN في label_num ──────────────────────
    invalid = df['label_num'].isna().sum()
    if invalid > 0:
        st.warning(f'⚠️ تم تجاهل {invalid} صف بسبب labels غير معروفة.')
...
def predict(text, model, tfidf):
    ...
    # ml_spam_prob: احتمال أن الرسالة spam حسب الموديل (بين 0 و 100)
    ml_spam_prob = ml_prob[1] * 100
    final = 1 if (kw_spam or ml_pred == 1) else 0
    return final, ml_pred, ml_spam_prob, matched_kw

# ── History persistence helpers ───────────────────────────────
HISTORY_FILE = 'spam_history_persistent.csv'
HISTORY_COLS = ['time', 'message', 'result', 'conf', 'keyword']

def load_history():
    """تحميل السجل من ملف CSV لو موجود."""
    try:
        df_h = pd.read_csv(HISTORY_FILE, encoding='utf-8-sig')
        df_h = df_h[HISTORY_COLS]          # نضمن ترتيب الأعمدة
...
def save_history(history):
    ...
    try:
        pd.DataFrame(history, columns=HISTORY_COLS).to_csv(
...
if 'history' not in st.session_state:
    st.session_state.history = load_history()   # ← تحميل من الملف عند أول تشغيل
...
with st.spinner('🔄 جاري تدريب الموديل، لحظة من فضلك... | Training model, please wait...'):
...
with tab1:
    ...
    ex_list = [
        ...
        ('✅ Ham AR (اجتماع)',   "هل ستحضر الاجتماع غداً؟ أرسل لي التقرير من فضلك قبل نهاية اليوم."),
        ('🚨 Spam AR (نصب)',    "مبروك! لقد فزت بجائزة كبيرة. أرسل بياناتك الآن للمطالبة بجائزتك!"),
        ('✅ Ham AR (شكر)',      "شكراً على مساعدتك بالأمس كثيراً. سأكون في المكتب من الساعة التاسعة."),
    ]
    ...
    # ── تعديل 3: حد أدنى للطول + تخزين النتيجة في session_state ──
    if analyze:
        if not user_input.strip():
            st.warning('⚠️ من فضلك اكتب رسالة أولاً | Please enter a message first.')
        else:
            if len(user_input.split()) < 3:
                st.warning('⚠️ الرسالة قصيرة جداً — النتيجة قد لا تكون دقيقة، أضف المزيد من الكلمات.\n'
                           'Message too short — result may be inaccurate. Please add more context.')
            final, ml_pred, ml_spam_prob, matched_kw = predict(user_input, model, tfidf)
            st.session_state.last_result = (final, ml_pred, ml_spam_prob, matched_kw, user_input)
            st.session_state.show_result = True

    # ── عرض النتيجة منفصل عن زرار Analyze عشان يفضل ظاهر ──────
    if st.session_state.get('show_result') and 'last_result' in st.session_state:
        final, ml_pred, ml_spam_prob, matched_kw, analyzed_text = st.session_state.last_result

        # ── تعديل 1: Confidence منطقي ────────────────────────────
        # لو keyword اتطابقت والموديل قال ham:
        #   نضيف 30% فوق احتمال الموديل بحد أقصى 85%
        #   عشان ميبقاش 100% hardcoded بدون مبرر
        if matched_kw and ml_pred == 0:
            spam_conf = min(85.0, ml_spam_prob + 30.0)
        else:
            spam_conf = ml_spam_prob
        spam_conf = round(max(0.0, min(100.0, spam_conf)), 1)
        ham_conf  = round(100.0 - spam_conf, 1)
        ...
        # ── تعديل 4: زرار "تحليل رسالة جديدة" يمسح النتيجة والنص ──
        st.divider()
        if st.button('🔄 تحليل رسالة جديدة | Analyze New Message', use_container_width=True):
            st.session_state.msg_input   = ''
            st.session_state.show_result = False
            st.session_state.pop('last_result', None)
            st.rerun()

        # ── تعديل 2: حفظ في history + CSV ───────────────────────
        # نحفظ بس لو الرسالة دي مش محفوظة بالفعل (نتفادى التكرار عند rerun)
        if (not st.session_state.history or
                st.session_state.history[0].get('message') !=
                analyzed_text[:80] + ('...' if len(analyzed_text) > 80 else '')):
            new_entry = {
                'time'   : datetime.now().strftime('%H:%M:%S'),
                'message': analyzed_text[:80] + ('...' if len(analyzed_text) > 80 else ''),
                'result' : 'SPAM' if final == 1 else 'HAM',
                'conf'   : f'{spam_conf:.1f}%' if final == 1 else f'{ham_conf:.1f}%',
                'keyword': matched_kw or '-',
            }
            st.session_state.history.insert(0, new_entry)
            save_history(st.session_state.history)   # ← حفظ فوري في CSV
...
with tab2:
    ...
    if not st.session_state.history:
        ...
        if btn_col1.button('🗑️ Clear History | مسح السجل'):
            st.session_state.history = []
            save_history([])   # ← امسح الـ CSV برضو
            st.rerun()
        df_hist = pd.DataFrame(st.session_state.history)
        csv_data = df_hist.to_csv(index=False).encode('utf-8-sig')
        btn_col2.download_button(
            label='⬇️ Export CSV | تنزيل السجل',
            data=csv_data,
...
| **English (SMS Spam)** | {metrics['en_count']:,} messages{' *(not loaded)*' if metrics['en_count'] == 0 else ''} |
